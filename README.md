...
- 상단의 전체 프로젝트 목록 펼침메뉴 > `새 프로젝트 만들기`
- 프로젝트 이름 : `적당한 이름` (중복체크)
- 조직 없음
...
- 승인된 리디렉션 URI : `https://Supabse프로젝트아이디.supabase.co/auth/v1/callback`
- 만들기 버튼 선택
...
- 왼쪽 최상단의 `햄버거 메뉴` 선택
- API 및 서비스 메뉴 선택 > Auth 동의 메뉴 선택 > 데이터액세스 선택
- `범위 추가 또는 삭제 버튼` 선택
...
- 프로젝트 > Auth > SignIn/Providers > Google > Enable > 값 항목 입력
- 저장 확인
