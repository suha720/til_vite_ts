...
- `Email 항목`이 활성화(Enabled) 되어 있는지 확인
...
- Confirm Singup 탭 : 회원가입 시 전달되는 인증메일 제목, 내용을 작성함.

### 1.4. URL Configuration 메뉴

- Site URL : `http://localhost:5173` (추후 Vercel 주소로 변경예정, 현재 Vite 주소임)
- Redirect URLs : `http://localhost:5173`, 또는 `http://localhost:3000` 등 Vite 나 CRA 중 선택
...
export const AuthProvider: React.FC<PropsWithChildren> = ({ children }) => {
  ...
  useEffect(() => {
    ...
    const { data } = supabase.auth.onAuthStateChange((_event, newSeesion) => {
      setSession(newSeesion);
      setUser(newSeesion?.user ?? null);
    });
  ...
  const signUp: AuthContextType['signUp'] = async (email, password) => {
    const { error } = await supabase.auth.signUp({
      ...
      password,
      options: {
        // 회원 가입 후 이메일로 인증 확인서 리다이렉트 될  URL
...
## 11. 새로 고침을 하거나, 직접 주소를 입력시에도 사용자 정보 유지하기
