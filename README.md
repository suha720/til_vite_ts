...
- 생성된 앱 `나의 할일 서비스` > 카카오 로그인 메뉴 선택 > 개인 개발자 비즈 앱 > 개인 개발자 비즈 앱 전환
- 플랫폼 > Web 플랫폼 등록 > 사이트 등록 > `http://localhost:5173`

### 1.2. Rest API 키를 보관.

- `e616e55dbff264764eebd8c5878a0183`

### 1.3. 사용 설정

- 카카오 로그인 메뉴 > 일반 > `사용 설정 활성`

### 1.4. 동의항목

- 카카오 로그인 메뉴 > 일반 >개인정보 > `닉네임, 프로필 사진, 카카오계정(이메일)` 활성화
...
- 시크릿 키 보관 : `vCIwXGrhP2TbGp8OYjFTjjhqUCyKjynj`

## 2. Supabase 인증 셋팅하기

- supabase.com > 프로젝트 > Authentication 메뉴 > Sign In / Providers 메뉴
- Auth Providers 항목에서 `kakao` 활성
- Rest API 키와 시크릿 키를 입력해둠.
- Callback URL (for OAuth) 의 내용을 반드시 보관해 둠.
  - https://whoricmpxkauhscitfgu.supabase.co/auth/v1/callback
- `Callback URL 를 카카오 개발자의 리다이렉트 URI 에 추가 저장해둠`

## 3. 카카오 회원 가입하기코드 적용하기
...
- /src/components/KakaoLoginButton.tsx 생성
...
- DB 명령을 직접 실행하는 형태로 구성이 필요. (`PostgreSQL Function`)
- 많이 위험함.
...
- Supabase 대시보드 > API 메뉴 > Stored Procedures 목록 확인
...
### 6.2. 시나리오 1번 : 카카오 회원가입하기 > 이메일 가입

### 6.3. 시나리오 2번 : 이메일 가입 > 카카오 회원가입하기

### 6.4. 시나리오 3번 : 관리자는 탈퇴를 못한다.

### 6.5. 시나리오 4번 : 비밀번호를 변경하기 또는 비밀번호 찾기

## 7. 버그 수정

- 카카오 로그인 시 인증코드가 제대로 작동 안되는 경우 발견
